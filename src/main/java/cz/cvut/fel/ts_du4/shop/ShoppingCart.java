package cz.cvut.fel.ts_du4.shop;

import java.util.ArrayList;


/**
 * Class for shopping cart. 
 *
 */

public class ShoppingCart {

    
    ArrayList<Item> items;

    public ShoppingCart(ArrayList<Item> items) {
        this.items = items;
    }
    
    public ShoppingCart() {
        items = new ArrayList<Item>();
    }

    /**
     * Gets items in the shopping cart.
     * @return   items in the shopping cart
     */
    public ArrayList<Item> getCartItems() {
        return items;
    }

    public void addItem(Item temp_item) {
        items.add(temp_item);
        System.out.println("Item with ID " + temp_item.getID() + " added to the shopping cart.");
    }

    
    /**
     * Removes item from the shopping chart
     * 
     * @param itemID   ID of the item to remove form the shopping chart
     */
    
    public void removeItem(int itemID) throws Exception {
        for (int index = 0; index < items.size(); index++) {
            if (items.get(index).getID() == itemID) {
                items.remove(index);
                System.out.println("Item with ID " + items.get(index).getID() + " removed from the shopping cart.");
                return;
            }
        }
        throw new Exception("Item is no longer in Cart");
    }

    public int getItemsCount() {
        return items.size();
    }

    
    /**
     * Gets total price with discount, if there are any discounted items in the chart
     * 
     * @return total price with discount
     */
    public int getTotalPrice() {
        int total = 0;
        for (int index = 0; index < items.size(); index++) {
            if (items.get(index).getClass().getSimpleName().equals("DiscountedItem")){
                DiscountedItem DiscItem = (DiscountedItem) items.get(index);
                total += DiscItem.getDiscountedPrice();
            } else {
                total += items.get(index).getPrice();
            }
        }
        return total;
    }
    
    
    
    
    
    
    
    
}
